...
**当前目标**：插入 egui 透明浮窗，用来在右键手势会话期间显示鼠标轨迹、方向/动作提示，以及位于展开点上方的功能按钮。按钮目前先做假功能：鼠标移动到按钮上时选中，右键抬起时发送对应执行包并打印文字，然后关闭 UI。

**计划状态**：已确认方案，计划文件为 `egui 手势浮窗`。当前阶段优先保持同步线程 + `std::sync::mpsc` 的简单可靠风格；Tokio 不在本阶段引入，后续外部通信模块需要异步 I/O 时再在模块内部局部封装。

## 当前计划待办

目标：先做一个可运行的 egui overlay 原型，使手势期间能看见轨迹、看见按钮选中、松开右键时能触发假动作；不要在本阶段扩展真实 tab 数据、浏览器扩展或复杂菜单系统。

- [ ] 接入 `eframe/egui` 透明浮窗，展示右键手势轨迹和少量功能按钮。
- [ ] 保持核心手势识别只有一份，Overlay 只展示事件和结果，不参与动作判断。
- [ ] 右键抬起时根据当前选中按钮发送假动作并关闭 UI。
- [ ] 保持当前同步线程架构简单可靠，完成后验证现有手势逻辑不回退。
...
- 浮窗窗口：egui 负责绘制，Win32 窗口层负责透明、无边框、置顶、不抢焦点；当前优先保证不抢焦点，穿透按需要再做。
