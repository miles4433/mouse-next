...
- `Hook` 将 `原始鼠标事件` 同时发给手势处理器和 overlay。
- 处理器继续输出方向事件，动作匹配链路保持唯一。
- Overlay 状态维护显示开关、锚点、轨迹点、按钮列表、选中按钮和提示文字。
- egui 每帧读取状态绘制，不在每个鼠标包触发 UI 刷新。
- 轨迹点做简单距离降采样。
- 执行器支持 `虚拟键操作::打印`，用于假功能验证。
- 字体从 `font/` 目录加载第一个 `.ttf/.otf/.ttc`。

当前调试状态：

- 常态绘制浅灰背景。
- 右键按下后绘制浅红背景和手势 UI。
- 窗口按当前鼠标所在显示器的 `rcWork` 放置。
- 仍在验证焦点和穿透问题。
